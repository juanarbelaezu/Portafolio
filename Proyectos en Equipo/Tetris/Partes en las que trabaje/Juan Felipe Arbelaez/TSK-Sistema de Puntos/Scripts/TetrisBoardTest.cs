...
public class TetrisBoardTest : MonoBehaviour
{
    ...
    private Points_System pointsystem; // Puntos
    [SerializeField] ParticleSystem rectangleparty; // Particulas Rectangulo
    ...
    void Start()
    {
        block = new Block[W, H];
        Generate();
        pointsystem = GameObject.Find("points").GetComponent<Points_System>();
    }
        ...
        for (int i = H - 1; i > 0; i--)
        {
            ...
            else
            {
                ...
                for (int n = 0; n < blockToClear.Count; n++)
                {
                    ...
                    rectangleparty.transform.position = new Vector3(4, blockToClear[n].y, -2);
                }                   
            }

            if(count==W)
            {
                pointsystem.truecombo();
                rectangleparty.Play();
            }

            for (int j = 0; j < W; j++) //set position
            {
                if (block[j, i].ob != null)
                {
